(function(ThemeBuilder) {
    ...
    var ViewModel = function(metadataRepository, historyChangesManager, previewIframeNotifier, cssBeautifier) {
        ...
        var getInheritedValues = function(metadata, key) {
            var result = [];
            $.each(metadata, function(_, group) {
                $.each(group, function(index, item) {
                    if(item.Inherit && item.Inherit === key) {
                        result.push({
                            group: item.Group,
                            index: index,
                            key: item.Key
        ...
        var updateInheritedValues = function(metadata, key, value) {
            var inheritedItems = getInheritedValues.call(this, metadata, key),
                that = this;

            if(inheritedItems.length) {
                $.each(inheritedItems, function(_, inheritedItem) {

                    var metadataValue = metadata[inheritedItem.group][inheritedItem.index].Value,
                        inheritedColorAlpha = new DevExpress.Color(metadataValue).a,
                        parentColor = new DevExpress.Color(value),
                        inheritedColor = "rgba(" + [parentColor.r, parentColor.g, parentColor.b, inheritedColorAlpha].join(",") + ")";

                    var treeItem = that.getTreeItemDataByKey(inheritedItem.key);
                    if(treeItem)
                        treeItem.value(inheritedColor);
                });
            }
        };
        ...
        this.toolboxValueSubscribe = function(item, value) {
            ...
            updateInheritedValues.call(this, metadata, item.Key, value);

            if(updatePreviewStyles) {
                previewIframeNotifier.notify($previewIframe, "update", {
                    metadata: metadata,
                    group: this.currentWidgetGroup(),
        ...
        this.exportMetadata = function() {
            ...
            this.metadataPopupOptions.visible(true);
            this.metadataPopupButtonsVisible(false);
        ...
        this.showMetadataPopup = function() {
            this.metadataPopupButtonsVisible(true);
            this.metadataPopupOptions.visible(true);
        ...
        this.metadataPopupOptions = {
            ...
            height: ko.computed(function() {
                return this.metadataPopupButtonsVisible() ? 500 : 450;
            }, this),
            width: 600,
            buttons: [
                {
